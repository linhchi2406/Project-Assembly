...
li $t4, 0x04 # check row 3
li $t5, 0x02 #check row 2
li $t6, 0x01 # check row 1
polling: sb $t3, 0($t1 ) # must reassign expected row
lb $a0, 0($t2) # read scan code of key button
print: li $v0, 34 # print integer (hexa)
syscall
polling1: sb $t4, 0($t1 ) # must reassign expected row
lb $a0, 0($t2) # read scan code of key button
print1: li $v0, 34 # print integer (hexa)
syscall
polling2: sb $t5, 0($t1 ) # must reassign expected row
lb $a0, 0($t2) # read scan code of key button
li $v0, 34 # print integer (hexa)
syscall
polling3: sb $t6, 0($t1 ) # must reassign expected row
lb $a0, 0($t2) # read scan code of key button
